...
| LLM 简历解析流水线 | 文件抽取/清洗(PDF 优先 `pdftotext -layout`,扫描件与 DOC/DOCX/DOC fallback Kimi Files API) → **parseResume** 产 summary 简报 + 基础字段 + tags/languages + skills/experience/educationHistory markdown 事实字段 |
...
│       └── lib/               # api / kimi(文件抽取清洗 + parseResume 简历事实抽取 + matchAgainstJob JD 联评) / derived(profileCompletion) / parseTaskStore(reparse + parse-and-create 异步任务) / idLookup / settings / interviewEvalTemplate(评分维度+字段映射+计算+sanitize) / interviewEvalExport(ExcelJS 模板填充)
├── web/                       # Vite 生产前端
