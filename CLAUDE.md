...
│       │                      # 三层架构:textin(文档层) / jev(评估层) / documents(文档统一入口) / prompts(Kimi 三套 prompt) / taxonomy(词表 JSON + 归一) / profile(normalize 闸门 + derive 派生) / jd(jdFacts 闸门) / evaluation(hardFilter / templates / questions / composite / report / legacyAdapter / pipeline)
├── tools/md2html/             # Markdown → 单文件 HTML 文档生成器(§9.1)
├── web/                       # Vite 生产前端
...
| `ResumeParse` | candidateId/attachmentKey/fileSha256/provider(textin\|pdftotext\|kimi-files)/providerVersion/markdown(≤200KB)/rawResultKey(R2 `parses/<cid>/<sha>.json`)/pageCount/durationMs | **文档层产物(2026-09-20)** · unique(candidateId,fileSha256,provider) · reparse 复用不重复向 TextIn 计费 |
| `CandidateEvaluation` | candidateId/jobId/resumeParseId/engine(jev\|legacy)/isCurrent/shadow/hardFilter json/jevRequest(questions,不存 state)/jevAnswers/dimensions/items/overallScore/classification(A-D)/reviewPriority(HIGH\|REVIEW\|LOW)/reasons/flags/minConfidence/report json/reportStatus/manualOverride/versions/costs | **评估层产物** · 每次评估一行,isCurrent 标当前;Candidate.jdMatch/risks/highlights/insights/matchedFor/againstFor/aiSuggestedTags/classification/reviewPriority 是它的反规范化快照 |
| `Candidate`(增量) | profile json(resume.v1)/derived json/profileVersion/profileWarnings/classification/reviewPriority | 结构化档案 + 派生指标;旧列由 `profileToLegacy` 派生保持一致 |
| `Job`(增量) | jdFacts json(jd.v1)/jdFactsVersion/evaluationModel json/evaluationModelVersion/evaluationModelUpdatedAt/evaluationModelSource(draft\|kimi_suggested\|manual) | JD 事实层 + 评价模型层(HR 拥有 tier/权重/题目措辞) |
| `SystemSetting` | key/value(AES-256-GCM)/encrypted/updatedBy | admin 系统配置:`kimi.*`(api_key/model/prompt/report_prompt/jd_schema_prompt)· `textin.*`(app_id/secret_code 加密,enabled,max_pages)· `jev.*`(api_key 加密,enabled,mode,model,base_url,timeout_ms,concurrency,cache_ttl_s)· `eval.*`(thresholds/default_weights/confidence_gate/report_policy/pii_strip) |
