...
> 状态:**方案设计稿(尚未实施)** · 编写日期 2026-05-30
> 关联系统:MESA Recruit(`server/` Fastify + R2 + 现有公开上传通道)
...
- 内部「简历收集群」内,任何成员发的 **pdf / doc / docx** 文件 → 自动入库为候选人(`tags=["待解析","公开上传"]`),admin 后续一键解析。
- 不要求改动 MESA 后端入库逻辑(复用现有公开上传通道)。
- 不暴露新公网端口、不改 Cloudflare。

### 1.3 非目标(本期不做)

- 不监听个人私聊 DM(飞书平台不允许机器人窥探用户私聊,详见 §7)。
- 不在含外部联系人的「外部群」自动入库(默认平台禁止,详见 §6)。
- 不在入库阶段自动跑 LLM 解析(沿用「降级入库 + 后续手动解析」策略,避免长任务阻塞)。
