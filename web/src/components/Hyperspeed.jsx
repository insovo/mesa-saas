// Hyperspeed 光速公路背景 — three.js + postprocessing(Bloom + SMAA)
// 移植自 reactbits Hyperspeed(用户提供 TSX 参考,此处去 TS 类型 → JSX)。
// 用法:<Hyperspeed />(absolute/fixed 容器内铺满);按住鼠标/触摸加速。
// 注意:
//   - three/postprocessing 在 vite manualChunks 中单独切 "three" chunk,仅登录页加载
//   - prefers-reduced-motion / WebGL 创建失败时由调用方兜底(本组件 try/catch 后静默不渲染)
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // WebGL 不可用(旧设备 / 被禁用)→ 静默降级为纯色背景,不影响登录
      if (import.meta.env.DEV) console.warn("[Hyperspeed] WebGL init failed:", e);
      appRef.current = null;
    }
  ...
  // 容器铺满父级;canvas 由 three 注入后靠 CSS 撑满(见 index.css .hyperspeed-host canvas)
  return <div className="hyperspeed-host" ref={hyperspeed}></div>;
};
